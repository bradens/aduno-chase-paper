...
The navigation bar is a powerful aspect of the application, as it shows the currently online users, which offers \textit{presence}, as discussed by Gutwin et al.\cite{GG02}.  Additionally, it shows unique badges of other users when they are modifying items, which offers solutions to the \textit{artifact}, and \textit{intent} sections of the framework.  This system of unique user badges and colors allows for \textit{identity}.  Once user \textit{action} is performed on a work item, the changes are propagated in real time to all other connected clients.  \textit{Authorship} is provided by user badges and work item history.  
